...
module fpga_top (
    ...
    // Keep bridge instantiated, but ignore it in this forced-record debug build.
    wire       ctrl_capture_enable_unused;
    wire [1:0] ctrl_mode_unused;
    wire [7:0] ctrl_frame_stride_unused;
    wire       ctrl_clear_counts_unused;
    wire       ctrl_snapshot_unused;
    ...
    esp32_ctrl_uart_min_bridge u_ctrl (
        ...
        .rx(RX),
        .tx(TX),
        .capture_enable(ctrl_capture_enable_unused),
        .mode(ctrl_mode_unused),
        .frame_stride(ctrl_frame_stride_unused),
        .clear_counts_pulse(ctrl_clear_counts_unused),
        .snapshot_pulse(ctrl_snapshot_unused)
    );

    // Forced debug config
    wire       record_en_ctrl      = 1'b1;
    wire       detect_en_ctrl      = 1'b0;
    wire [7:0] record_stride_ctrl  = 8'd1;
    wire [7:0] detect_decim_ctrl   = 8'd0;
    wire       clear_counts_pulse  = 1'b0;
    ...
    always @(posedge clk_sys) begin
        if (!rst_sys_n) begin
            ...
            vsync_s2 <= 1'b1;
            vsync_s3 <= 1'b1;
            frame_counter_sys <= 32'd0;
        end else begin
            vsync_s1 <= VSYNC;
            vsync_s2 <= vsync_s1;
            vsync_s3 <= vsync_s2;
            if ((vsync_s3 == 1'b1) && (vsync_s2 == 1'b0))
                frame_counter_sys <= frame_counter_sys + 32'd1;
        end
    end
    wire frame_tick_sys = (vsync_s3 == 1'b1) && (vsync_s2 == 1'b0);
    assign status_frame_id = frame_counter_sys;

    // Detect decimator kept for build consistency
    ...
    always @(posedge PCLK) begin
        if (!rst_pclk_n) begin
            ...
            det_en_p1 <= detect_en_ctrl;
            det_en_p2 <= det_en_p1;
            det_decim_p1 <= detect_decim_ctrl;
            det_decim_p2 <= det_decim_p1;
            vsync_d_p <= VSYNC;
    ...
    raw_frame_capture #(.FRAME_W(FRAME_W), .FRAME_H(FRAME_H)) u_raw_cap (
        ...
        .capture_enable(record_en_ctrl),
        .frame_stride(record_stride_ctrl),
        .frame_ready(raw_frame_ready),
        .frame_id(raw_frame_id),
        .frame_w(raw_frame_w),
        .frame_h(raw_frame_h),
        .payload_bytes(raw_payload_bytes),
        .stream_kind(raw_stream_kind),
        .sdram_wr_valid(raw_sdram_wr_valid),
        .sdram_wr_data(raw_sdram_wr_data),
        .sdram_wr_ready(raw_sdram_wr_ready),
        .sdram_level_words(sdram_level_words)
    );
    ...
    sdram_ctrl_simple u_sdram (
        .clk(clk_sys),
        .rst_n(rst_sys_n),
        .wr_valid(raw_sdram_wr_valid),
        .wr_data(raw_sdram_wr_data),
        .wr_ready(raw_sdram_wr_ready),
        .rd_ready(sdram_rd_ready),
        .rd_valid(sdram_rd_valid),
        .rd_data(sdram_rd_data),
        .level_words(sdram_level_words),
        .SD_DQ(SD_DQ),
        .SD_DQM(SD_DQM),
        .SD_CLK(SD_CLK),
        .SD_CS_N(SD_CS_N),
    ...
    // Keep rest of pipeline instantiated so design matches project expectations
    ...
    motion_block_frontend #(.FRAME_W(FRAME_W), .FRAME_H(FRAME_H), .BX(40), .BY(30)) u_motion_frontend (
        .pclk(PCLK), .rst_pclk_n(rst_pclk_n), .cam_ok(cam_ok_pclk), .vsync(VSYNC), .href(HREF), .cam_d(cam_d),
        .enable(detect_do_frame_p), .frame_start_pulse(mf_frame_start), .frame_end_pulse(mf_frame_end),
    ...
    always @(posedge clk_sys) begin
        if (!rst_sys_n) begin
            ...
            props_tog_s1 <= props_tog_p;
            props_tog_s2 <= props_tog_s1;
        end
    end
    ...
    cropper_128_to_64 #(.FRAME_W(FRAME_W), .FRAME_H(FRAME_H), .N(N), .K(K)) u_cropper (
        ...
        .vsync(VSYNC), .href(HREF), .cam_d(cam_d), .props_ready(props_ready_sys),
        .prop_score_flat(prop_score_bus), .prop_cx_flat(prop_cx_bus), .prop_cy_flat(prop_cy_bus),
        .sel_cx_out_flat(sel_cx_bus), .sel_cy_out_flat(sel_cy_bus), .sel_sc_out_flat(sel_sc_bus),
        .out_start(cnn_in_start), .out_valid(cnn_in_valid), .out_data(cnn_in_data), .out_last(cnn_in_last),
        .out_k_idx(cnn_in_k_idx), .out_ready(cnn_in_ready)
    );
    ...
    cnn_scheduler #(.K(K), .C1(4), .PERSON_THRESH(8'd160)) u_sched (
        .clk(clk_sys), .rst_n(rst_sys_n), .in_start(cnn_in_start), .in_valid(cnn_in_valid), .in_data(cnn_in_data),
        .in_last(cnn_in_last), .in_k_idx(cnn_in_k_idx), .in_ready(cnn_in_ready), .scores_ready(scores_ready),
        .scores_k_idx(scores_k_idx), .score_person_flat(score_person), .is_person_flat(is_person)
    );

    wire [K*16-1:0] cand_x0, cand_y0, cand_x1, cand_y1;
    wire [K*8-1:0]  cand_score_motion, cand_score_person, cand_flags, cand_track_id;
    candidate_bus #(
        .K(K), .FRAME_W(FRAME_W), .FRAME_H(FRAME_H), .MAX_T(8),
        .X_LINE(16'd320), .YA0(16'd200), .YA1(16'd430), .YB0(16'd80), .YB1(16'd180), .GATE_R(16'd60)
    ) u_candidate_bus (
        .clk(clk_sys), .rst_n(rst_sys_n), .frame_tick(frame_tick_sys), .clear_counts(clear_counts_pulse),
        .cand_set_ready(props_ready_sys), .sel_cx_flat(sel_cx_bus), .sel_cy_flat(sel_cy_bus), .sel_sc_flat(sel_sc_bus),
        .scores_ready(scores_ready), .scores_k_idx(scores_k_idx), .score_person_flat(score_person), .is_person_flat(is_person),
        .cand_x0_flat(cand_x0), .cand_y0_flat(cand_y0), .cand_x1_flat(cand_x1), .cand_y1_flat(cand_y1),
        .cand_score_motion_flat(cand_score_motion), .cand_score_person_flat(cand_score_person),
        .cand_flags_flat(cand_flags), .cand_track_id_flat(cand_track_id),
    ...
    sd_spi_writer u_sd (
        ...
        .sd_ready(sd_ready), .sd_err(sd_err), .w_start(sd_w_start), .w_lba(sd_w_lba), .w_data_valid(sd_w_data_valid),
        .w_data(sd_w_data), .w_data_ready(sd_w_data_ready), .w_done(sd_w_done), .w_busy(sd_w_busy)
    );
    ...
    frame_packer #(.K(K)) u_pack (
        .clk(clk_sys), .rst_n(rst_sys_n), .sd_ready(sd_ready), .frame_ready(pack_frame_ready), .frame_id(pack_frame_id),
        .payload_bytes(pack_payload_bytes), .stream_kind(pack_stream_kind), .ctrl_flags(pack_ctrl_flags),
        .record_stride_cfg(record_stride_ctrl), .detect_decim_cfg(detect_decim_ctrl),
        .cand_x0_flat(cand_x0), .cand_y0_flat(cand_y0), .cand_x1_flat(cand_x1), .cand_y1_flat(cand_y1),
        .cand_score_motion_flat(cand_score_motion), .cand_score_person_flat(cand_score_person),
        .cand_flags_flat(cand_flags), .cand_track_id_flat(cand_track_id),
        .count_A_LR(count_A_LR), .count_A_RL(count_A_RL), .count_B_LR(count_B_LR), .count_B_RL(count_B_RL),
        .total_people(total_people), .src_rd_ready(sdram_rd_ready), .src_rd_valid(sdram_rd_valid), .src_rd_data(sdram_rd_data),
        .sd_w_start(sd_w_start), .sd_w_lba(sd_w_lba), .sd_w_data_valid(sd_w_data_valid), .sd_w_data(sd_w_data),
        .sd_w_data_ready(sd_w_data_ready), .sd_w_done(sd_w_done), .sd_w_busy(sd_w_busy)
    );

    reg vs_m1, vs_m2;
    reg hr_m1, hr_m2;
    reg pc_m1, pc_m2;
    reg ev_vsync_seen, ev_href_seen, ev_pclk_seen;
    always @(posedge mclk_24 or negedge rst_sys_n) begin
        if (!rst_sys_n) begin
            vs_m1 <= 1'b1; vs_m2 <= 1'b1;
            hr_m1 <= 1'b0; hr_m2 <= 1'b0;
            pc_m1 <= 1'b0; pc_m2 <= 1'b0;
            ev_vsync_seen <= 1'b0;
            ev_href_seen  <= 1'b0;
            ev_pclk_seen  <= 1'b0;
        end else begin
            vs_m1 <= VSYNC; vs_m2 <= vs_m1;
            hr_m1 <= HREF;  hr_m2 <= hr_m1;
            pc_m1 <= PCLK;  pc_m2 <= pc_m1;
            if (vs_m2 ^ vs_m1) ev_vsync_seen <= 1'b1;
            if (hr_m2 ^ hr_m1) ev_href_seen  <= 1'b1;
            if (pc_m2 ^ pc_m1) ev_pclk_seen  <= 1'b1;
        end
    end

	wire dbg_tx;
	fpga_debug_status_uart_v2 #(
		.CLK_HZ(24000000),
		.BAUD(115200)
	) u_dbg_status (
		.clk(mclk_24), .rst_n(rst_sys_n),
		.cap(1'b1), .rec(record_en_ctrl), .det(1'b0), .cam_init_done(cam_init_done),
		.vsync_seen(ev_vsync_seen), .href_seen(ev_href_seen), .pclk_seen(ev_pclk_seen),
		.frame_tick_seen(ev_frame_tick_seen), .wr_valid_seen(ev_wr_valid_seen),
		.sdram_nonzero(|sdram_level_words), .frame_id_lsb(raw_frame_id[0]),
		.sd_ready(sd_ready), .sd_err(sd_err), .raw_frame_ready(raw_frame_ready),
		.sd_w_start(sd_w_start), .sd_w_busy(sd_w_busy), .sd_w_done(sd_w_done), .tx(dbg_tx)
	);
	
    assign ESP_D[0]   = dbg_tx;
    assign ESP_D[7:1] = 7'bzzzzzzz;
endmodule
