module w9825_ref_sdram_ctrl #(
    ...
    localparam [19:0] T_READ_OBS = 20'd31;
    ...
    localparam [4:0] ST_ACT       = 5'd6;
    localparam [4:0] ST_WRITE     = 5'd7;
    localparam [4:0] ST_READ      = 5'd8;
    localparam [4:0] ST_READ_LAT  = 5'd9;
    localparam [4:0] ST_PRE_CLOSE = 5'd10;
    localparam [4:0] ST_DQ_CLEAR  = 5'd11;
    localparam [4:0] ST_WAIT      = 5'd12;
    localparam [4:0] ST_ACK       = 5'd13;
    localparam [4:0] ST_REQ_LOW   = 5'd14;
            ...
            case (state)
                ST_INIT_WAIT: begin
                    if (timer == 20'd0) begin
                        ...
                        SD_BA <= 2'd0;
                        wait_then(ST_PRE_ALL, T_A10_SETUP);
                    end else begin
                        timer <= timer - 20'd1;
                    end
                end
                ...
                ST_REFRESH: begin
                    ...
                    if (!init_done) begin
                        if (refresh_count == 4'd7) begin
                            refresh_count <= 4'd0;
                            wait_then(ST_MRS, T_RFC);
                        end else begin
                            refresh_count <= refresh_count + 4'd1;
                            wait_then(ST_REFRESH, T_RFC);
                        end
                    end else begin
                        ref_ctr <= 20'd0;
                        wait_then(ST_IDLE, T_RFC);
                    end
                end
                ...
                ST_ACT: begin
                    ...
                    SD_DQM <= 2'b00;
                    dq_out <= op_wdata;
                    write_drive <= !op_is_read;
                    clear_drive <= 1'b0;
                    wait_then(op_is_read ? ST_READ : ST_WRITE, T_RCD);
                end
                ...
                ST_PRE_CLOSE: begin
                    ...
                    if (!op_is_read && (WRITE_CLEAR_CYCLES != 0))
                        wait_then(ST_DQ_CLEAR, T_RP);
                    else
                        wait_then(ST_ACK, T_RP);
                end
                ...
                ST_DQ_CLEAR: begin
                    ...
                    write_drive <= 1'b0;
                    clear_drive <= 1'b1;
                    wait_then(ST_ACK, T_DQ_CLEAR);
                end

                ST_WAIT: begin
                    if (timer == 20'd0) begin
                        state <= next_state;
                        if (next_state == ST_ACK)
                            clear_drive <= 1'b0;
