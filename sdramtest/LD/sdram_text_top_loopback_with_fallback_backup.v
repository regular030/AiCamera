module sdram_text_top #(
    ...
    function [5:0] free_sym;
        ...
        begin
            case (idx)
                ...
                3'd5: free_sym = 6'd3;
                3'd6: free_sym = 6'd63;
                default: free_sym = 6'd0;
            endcase
        end
    endfunction

    reg [23:0] free_ctr = 24'd0;
    always @(posedge clk) begin
        free_ctr <= free_ctr + 24'd1;
    end
    ...
    // Physical D3/GPIO16 is confirmed stuck low on this board path, so the
    // transport uses only the other six data pins. When the command TX path is
    // idle or held in reset, emit an independent M1-clock heartbeat so the bus
    // never looks dead while debugging SDRAM.
    wire tx_driving = !rst && ((tx_state != TX_IDLE) || ping_pending || read_pending || out_busy);
    wire [5:0] bus_sym6 = tx_driving ? out_sym6 : free_sym(free_ctr[18:16]);
    wire bus_strobe = tx_driving ? out_strobe : free_ctr[15];
    wire [6:0] out_bus7 = {bus_sym6[5], bus_sym6[4], bus_sym6[3], 1'b0,
                           bus_sym6[2], bus_sym6[1], bus_sym6[0]};

    assign ESP_D = {bus_strobe, out_bus7};
    assign DOUT = bus_strobe;
    assign TX = 1'b0;
endmodule
